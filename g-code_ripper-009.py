...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.probe_soft.set("LinuxCNC") # "LinuxCNC" or "MACH3"
...
#        self.probe_istep.trace_variable("w", self.Entry_ProbeIStep_Callback)
        ...
        self.ProbeSoft_OptionMenu = OptionMenu(root, self.probe_soft, "LinuxCNC", "MACH3")
...
################################################################################
        ...
        for line in fin:
            ...
            for cnt in range(0,3):
                try:
                    ...
                    fmessage("Input text: %s" %(values_text[cnt]))
                    return
    ...
    def WriteGCode(self,side="base",Rstock=0.0,Wrap="XYZ"):
        ...
        if (side == "probe_n_cut") or (side == "probe_only") or (side =="probe_adjusted"):
            ...
            if (side == "probe_n_cut") or (side == "probe_adjusted"):  #scorchworks
    ...
    def Plot_Arc(self,dir,XX1,YY1,ZZ1, XX2,YY2,ZZ2, xxc,yyc,midx,midy,cszw,cszh,PlotScale,col,radius=0):
        ...
        for at in my_range:
            ...
            self.Plot_Line_ISO(XX1,YY1,ZZ1,XX2,YY2,ZZ2,midx,midy,cszw,cszh,PlotScale,col,radius=0,dash=False)
...
##            self.Plot_Line_ISO(  minx,miny,minz,minx,miny,maxz, midH,midV,cszw,cszh,PlotScale,color,radius=rad)
    ...
    def DoIt(self):
        ...
        try:            
            ...
            scale_f      = float(self.SCALEZ.get() )/100.0
        ...
        if self.gcode_op.get() == "split":
            ...
            if (self.probe_data==[]):
                ...
                #print self.probe_data[0]
                min_probe_x = self.probe_data[0][0] - Xoffset # Should this be negative?
                min_probe_y = self.probe_data[0][1] - Yoffset # Should this be negative?

                last_point = len(self.probe_data)-1
                #print last_point
                max_probe_x = self.probe_data[last_point][0] - Xoffset # Should this be negative?
                max_probe_y = self.probe_data[last_point][1] - Yoffset # Should this be negative?
            ...
            self.g_rip.probe_code(self.g_rip.scaled_trans,nX,nY,probe_istep,min_probe_x,min_probe_y,xPartitionLength,yPartitionLength) #,DX,DY)
            ...
            if (self.probe_data==[]):
                #probe_coords([POINT_LIST[i],i,xp,yp,zp])
                ...
                for line in self.probe_data:
                    Xoffset = float(self.probe_offsetX.get())
                    Yoffset = float(self.probe_offsetY.get())
                    xp  = line[0] - Xoffset #scorchworks Should these be negative? YES
                    yp  = line[1] - Yoffset #scorchworks Should these be negative? YES
                    zp  = line[2]
                    Lpt= xPartitionLength/5
                    self.coords.append([xp-Lpt, yp    , zp, xp+Lpt, yp    , zp, 2, 1 ])
                    self.coords.append([xp    , yp+Lpt, zp, xp    , yp-Lpt, zp, 2, 1 ])
...
class G_Code_Rip:
    ...
    def Read_G_Code(self,filename, XYarc2line = False, arc_angle=10, units="in", Accuracy=""):
        ...
        for line in fin:
                ...
                while s != -1:
                    ...
                    if e < len(line):
                        if line[e]=="=":
                            DEFINE = True
                    if DEFINE:
                        #vval = re.findall(r'[-+]\d*\.?\d+(?:[Ee][+-]?\d+)?$', line[e+1:])[0]
                        try:
            ...
            for code in UCODES:
                if line.find(code) != -1:
                    READ_MSG.append("Aborting G-Code Reading: %s Codes are not supported" %(code))
                    return READ_MSG
    ...
    def generate_probing_gcode(self,
        ...
        g_code.append("( by Scorch - 2015 www.scorchworks.com                    )")
        if self.units == "in":
...
###
    ...
    def get_arc_intersects(self, p1, p2, xsplit, cent, code):
        ...
        if abs(R-Rt) > self.accuracy:  print("Radius Warning: R1=%f R2=%f"%(R,Rt))
    ...
    def EXPRESSION_EVAL(self,line):
        ...
        cnt = 1
        while cnt > 0:
            if (not cmp(line[cnt],'+')) or (not cmp(line[cnt],'-')):
                sign = 1
                span = 0
                FLAG = True
                while FLAG:
                    if not cmp(line[cnt+span],'+'):
                        span = span + 1
                    elif not cmp(line[cnt+span],'-'):
                        sign = -sign
                        span = span + 1
                    else:
                        FLAG = False
        ...
        while s != -1:
            ...
            for cnt in range(0,len(temp)):
                if (not cmp(temp[cnt],'^')) or \
                   (not cmp(temp[cnt],'*')) or \
                   (not cmp(temp[cnt],'%')):
                    if not cmp(temp[cnt+1],'-'):
                        temp[cnt+1]=''
                        temp[cnt+2]= -float(temp[cnt+2])
                    elif not cmp(temp[cnt+1],'+'):
                        temp[cnt+1]=''
                        temp[cnt+2]= float(temp[cnt+2])
            ...
            for iXOR in range(0,len(XOR_operation)):
                ...
                for iAND in range(0,len(AND_operation)):
                    ...
                    for iLE in range(0,len(LE_operation)):
                        ...
                        for iLT in range(0,len(LT_operation)):
                            ...
                            for iGE in range(0,len(GE_operation)):
                                ...
                                for iGT in range(0,len(GT_operation)):
                                    ...
                                    for iNE in range(0,len(NE_operation)):
                                                        ...
                                                        for idiv in range(0,len(divide)):
                                                            ...
                                                            for imod in range(0,len(mod)):
                                                                ...
                                                                for k in range(1,len(power)):
                                                                    ...
                                                                    if P==True: print   "  POWER",power,"=",res_power
                                                                mod[imod]=res_power
                                                            #####################################
                                                            #REVERSE MOD
                                        ...
                                        for k in range(1,len(EQ_operation),1):
                                            if res_EQ == EQ_operation[k]:
                                                ...
                                                res_EQ = 0
                                            if P==True: print   "      EQ",EQ_operation,"=",res_EQ
                                    ...
                                    for k in range(1,len(NE_operation),1):
                                        if res_NE != NE_operation[k]:
                                            ...
                                            res_NE = 0
                                        if P==True: print   "      NE",NE_operation,"=",res_NE
                                ...
                                for k in range(1,len(GT_operation),1):
                                    if res_GT > GT_operation[k]:
                                        ...
                                        res_GT = 0
                                    if P==True: print   "      GT",GT_operation,"=",res_GT
                            ...
                            for k in range(1,len(GE_operation),1):
                                if res_GE >= GE_operation[k]:
                                    ...
                                    res_GE = 0
                                if P==True: print   "      GE",GE_operation,"=",res_GE
                        ...
                        for k in range(1,len(LT_operation),1):
                            if res_LT < LT_operation[k]:
                                ...
                                res_LT = 0
                            if P==True: print   "      LT",LT_operation,"=",res_LT
                    ...
                    for k in range(1,len(LE_operation),1):
                        if res_LE <= LE_operation[k]:
                            ...
                            res_LE = 0
                        if P==True: print   "      LE",LE_operation,"=",res_LE
                ...
                for k in range(1,len(AND_operation),1):
                    if res_AND and AND_operation[k]:
                        ...
                        res_AND = 0
                    if P==True: print   "      AND",AND_operation,"=",res_AND
            ...
            for k in range(1,len(XOR_operation),1):
                if bool(res_XOR) ^ bool(XOR_operation[k]):
                    ...
                    res_XOR = 0
                if P==True: print   "      XOR",XOR_operation,"=",res_XOR
